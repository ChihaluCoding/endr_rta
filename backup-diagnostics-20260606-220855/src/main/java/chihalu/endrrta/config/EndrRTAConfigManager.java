...
public final class EndrRTAConfigManager {
	...
	public static void load() {
		...
		try (Reader reader = Files.newBufferedReader(CONFIG_PATH)) {
			EndrRTAConfig loaded = GSON.fromJson(reader, EndrRTAConfig.class);
			config = loaded == null ? new EndrRTAConfig() : loaded;
		} catch (IOException exception) {
			EndrRTA.LOGGER.warn("Failed to read EndrRTA config. Defaults will be used.", exception);
	...
	public static void save() {
		try {
			...
			EndrRTA.LOGGER.warn("Failed to write EndrRTA config.", exception);
		}
	}
}
